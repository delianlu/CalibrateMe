import { useState, useEffect, useRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Bot, BookOpen, Lightbulb, AlertTriangle, Loader2, Volume2 } from 'lucide-react';
import { aiService } from '../../../services/aiService';

interface VocabularyExplanationProps {
  word: string;
  translation: string;
  userConfidence: number;
  tags?: string[];
}

interface ExplanationData {
  explanation: string;
  etymology: string;
  usageNotes: string;
  mnemonic: string;
  commonConfusions: string;
  exampleSentences: string[];
}

const SESSION_CACHE_PREFIX = 'calibrateme_vocab_explain_';
const MAX_EXPLANATIONS_PER_SESSION = 10;
const SESSION_COUNT_KEY = 'calibrateme_vocab_explain_count';

function getSessionCount(): number {
  try {
    return parseInt(sessionStorage.getItem(SESSION_COUNT_KEY) || '0', 10);
  } catch { return 0; }
}

function incrementSessionCount(): void {
  try {
    const current = getSessionCount();
    sessionStorage.setItem(SESSION_COUNT_KEY, String(current + 1));
  } catch { /* ignore */ }
}

function getCacheKey(word: string): string {
  return SESSION_CACHE_PREFIX + btoa(encodeURIComponent(word)).slice(0, 40);
}

function getCachedExplanation(word: string): ExplanationData | null {
  try {
    const key = getCacheKey(word);
    const raw = sessionStorage.getItem(key);
    if (raw) return JSON.parse(raw);
  } catch { /* ignore */ }
  return null;
}

function cacheExplanation(word: string, data: ExplanationData): void {
  try {
    const key = getCacheKey(word);
    sessionStorage.setItem(key, JSON.stringify(data));
  } catch { /* ignore */ }
}

export default function VocabularyExplanation({
  word,
  translation,
  userConfidence,
  tags,
}: VocabularyExplanationProps) {
  const [explanation, setExplanation] = useState<ExplanationData | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(false);
  const [rateLimited, setRateLimited] = useState(false);
  const fetchedRef = useRef(false);

  useEffect(() => {
    if (fetchedRef.current) return;
    fetchedRef.current = true;

    const cached = getCachedExplanation(word);
    if (cached) {
      setExplanation(cached);
      return;
    }

    if (getSessionCount() >= MAX_EXPLANATIONS_PER_SESSION) {
      setRateLimited(true);
      return;
    }

    const fetchExplanation = async () => {
      setLoading(true);
      try {
        const result = await aiService.explainVocabulary({
          word,
          translation,
          userConfidence,
          tags,
        });
        setExplanation(result);
        cacheExplanation(word, result);
        incrementSessionCount();
      } catch {
        setError(true);
      } finally {
        setLoading(false);
      }
    };

    fetchExplanation();
  }, [word, translation, userConfidence, tags]);

  if (rateLimited) {
    return (
      <div className="grammar-ai-explain grammar-ai-explain--limited">
        <p>AI explanations used for this session. Review your mistakes in the session summary.</p>
      </div>
    );
  }

  if (loading) {
    return (
      <div className="grammar-ai-explain grammar-ai-explain--loading">
        <Loader2 size={16} className="grammar-ai-explain__spinner" />
        <span>Getting AI vocabulary explanation...</span>
      </div>
    );
  }

  if (error || !explanation) return null;

  return (
    <AnimatePresence>
      <motion.div
        className="grammar-ai-explain"
        initial={{ opacity: 0, height: 0 }}
        animate={{ opacity: 1, height: 'auto' }}
        transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
      >
        <div className="grammar-ai-explain__header">
          <Bot size={16} />
          <span>AI Vocabulary Coach</span>
        </div>

        <div className="grammar-ai-explain__section">
          <div className="grammar-ai-explain__label">
            <BookOpen size={14} />
            <span>Meaning &amp; Usage</span>
          </div>
          <p>{explanation.explanation}</p>
        </div>

        <div className="grammar-ai-explain__section">
          <div className="grammar-ai-explain__label">
            <Volume2 size={14} />
            <span>Etymology</span>
          </div>
          <p>{explanation.etymology}</p>
        </div>

        <div className="grammar-ai-explain__section">
          <div className="grammar-ai-explain__label">
            <Lightbulb size={14} />
            <span>Usage Notes</span>
          </div>
          <p>{explanation.usageNotes}</p>
        </div>

        <div className="grammar-ai-explain__section grammar-ai-explain__section--tip">
          <div className="grammar-ai-explain__label">
            <Lightbulb size={14} />
            <span>Memory Trick</span>
          </div>
          <p>{explanation.mnemonic}</p>
        </div>

        <div className="grammar-ai-explain__section">
          <div className="grammar-ai-explain__label">
            <AlertTriangle size={14} />
            <span>Common Confusions</span>
          </div>
          <p>{explanation.commonConfusions}</p>
        </div>

        {explanation.exampleSentences?.length > 0 && (
          <div className="grammar-ai-explain__section">
            <div className="grammar-ai-explain__label">
              <BookOpen size={14} />
              <span>Examples</span>
            </div>
            <ul style={{ margin: '4px 0', paddingLeft: 16 }}>
              {explanation.exampleSentences.map((s, i) => (
                <li key={i} style={{ fontSize: 13, marginBottom: 2 }}>{s}</li>
              ))}
            </ul>
          </div>
        )}

        {userConfidence > 70 && (
          <div className="grammar-ai-explain__section grammar-ai-explain__section--overconfidence">
            <p>
              You were {userConfidence}% confident but got this wrong. The word may look or sound
              familiar from French, but has different nuances in English. Try recalling the exact
              definition before rating confidence next time.
            </p>
          </div>
        )}
      </motion.div>
    </AnimatePresence>
  );
}
